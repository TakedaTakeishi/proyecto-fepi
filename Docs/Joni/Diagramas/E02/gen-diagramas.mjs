// Generador de los diagramas SVG de E-02 (estadística y evaluación de temporada)
...
// ------------------------------------- E2·A CIERRE DE TEMPORADA E INFORME ANUAL
{
  const d = new Diagram({
    title: 'E2·A — Cierre de temporada: informe anual ante la Instancia Normativa',
    lanes: [
      { id: 'R37', name: 'Área del programa' }, { id: 'R19·R21', name: 'Sec. Técnica del Comité' },
      { id: 'R38·R32', name: 'Comité (Normativa)' }, { id: 'R15', name: 'DAFGD' }, { id: 'web', name: 'Sitio · Finanzas (destino)' },
    ],
    laneW: 216, gutter: 24, bottom: 640,
  });
  d.node('start', 0, 108, [], 'start');
  d.node('A1', 0, 158, ['A1 · Consolida por', 'beneficiario (pagos, minutas)']);
  d.node('A2', 0, 232, ['A2 · Elabora informe anual', 'de ejecución']);
  d.node('A2b', 3, 232, ['· cifras de presupuesto', 'autorizado/ejercido']);
  d.node('A3', 1, 306, ['A3 · Convoca sesión con', 'documentación (3 días)']);
  d.node('A4', 2, 380, ['A4 · Autoriza el Informe', 'Anual (acta)']);
  d.node('A5', 4, 454, ['A5 · Difunde resultados +', 'formato ayudas/subsidios']);
  d.node('end', 4, 520, [], 'end');
  d.edge('start', 'A1');
  d.edge('A1', 'A2');
  d.edge('A2', 'A3');
  d.edge('A2b', 'A3', { exit: 'bottom', enter: 'top', midY: 272 });
  d.edge('A3', 'A4');
  d.edge('A4', 'A5');
  d.edge('A5', 'end');
  writeFileSync(process.argv[2] + '/E2-A-informe-anual.svg', d.render(), 'utf8');
}

// ----------------------------------------- E2·B CICLO ESTATAL DE EVALUACIÓN
{
  const d = new Diagram({
    title: 'E2·B — Ciclo estatal de evaluación del Pp. 03020201 "Desarrollo Forestal"',
    lanes: [
      { id: 'R39', name: 'Evaluadora externa' }, { id: 'R21', name: 'UIPPE' },
      { id: 'R37·R15', name: 'Áreas · DAFGD' }, { id: 'web', name: 'Difusión' },
    ],
    laneW: 216, gutter: 24, bottom: 700,
  });
  d.node('start', 0, 108, [], 'start');
  d.node('B1', 0, 158, ['B1 · Emite PAE; elige', 'Pp./tipo de evaluación']);
  d.node('B2', 1, 232, ['B2 · Designa persona de', 'seguimiento']);
  d.node('B3', 2, 306, ['B3 · Entrega MIR,', 'presupuesto, padrones']);
  d.node('B4', 0, 380, ['B4 · Informe con hallazgos', 'y aspectos de mejora']);
  d.node('B5', 1, 454, ['B5 · Convenio de mejora +', 'acta de cumplimiento']);
  d.node('B6', 3, 528, ['B6 · Publica Anexo 1', '(→ E-04 SIED)']);
  d.node('end', 3, 594, [], 'end');
  d.edge('start', 'B1');
  d.edge('B1', 'B2');
  d.edge('B2', 'B3');
  d.edge('B3', 'B4');
  ...
  d.edge('B5', 'B6');
  d.edge('B6', 'end');
  writeFileSync(process.argv[2] + '/E2-B-evaluacion-estatal.svg', d.render(), 'utf8');
}
console.log('OK');